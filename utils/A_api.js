...
export const getStudentsByFilters = ({
  department,
  class: studentClass,
  section,
}) => {
...
export const getUsers = async (queryParams = {}) => {
  ...
  if (departmentId) {
    userData = userData.filter(
      (user) => user.departmentId === parseInt(departmentId)
    );
  }
  ...
  if (sectionId && role === 'student') {
    userData = userData.filter(
      (user) => user.sectionId === parseInt(sectionId)
    );
  }
...
export const getClasses = async (departmentId) => {
  ...
  if (departmentId) {
    classData = classData.filter(
      (cls) => cls.departmentId === parseInt(departmentId)
    );
  }
...
export const getSections = async (classId) => {
  ...
  if (classId) {
    sectionData = sectionData.filter(
      (section) => section.classId === parseInt(classId)
    );
  }
...
export const updateCourse = async (courseId, courseData) => {
  ...
  const department = mockDepartments.find(
    (d) => d.id === updatedCourse.departmentId
  );
...
export const getSchedules = async (queryParams = {}) => {
  ...
  const { view_type, departmentId, classId, sectionId, facultyId, day } =
    queryParams;
  ...
  if (view_type === 'student') {
    ...
    if (facultyId) {
      schedules = schedules.filter(
        (schedule) => schedule.facultyId === parseInt(facultyId)
      );
    }
...
export const getAttendance = async (queryParams = {}) => {
  ...
  if (view_type === 'student') {
    ...
    if (departmentId || classId || sectionId) {
      attendance = attendance.filter((a) => {
        ...
        if (departmentId && student.departmentId !== parseInt(departmentId))
          return false;
        if (classId && student.classId !== parseInt(classId)) return false;
        if (sectionId && student.sectionId !== parseInt(sectionId))
          return false;
    ...
    if (facultyId) {
      ...
      attendance = attendance.filter((a) =>
        facultyCourses.includes(a.courseId)
      );
    }
  }
  ...
  const attendanceWithRelated = attendance.map((record) => {
    ...
    const faculty = mockFaculty.find((f) =>
      mockSchedules.some(
        (s) => s.courseId === record.courseId && s.facultyId === f.id
      )
    );
...
export const updateAttendance = async (attendanceId, data) => {
  ...
  const student = mockStudents.find(
    (s) => s.id === updatedAttendance.studentId
  );
...
export const getInvoices = async (queryParams = {}) => {
  ...
  if (studentId) {
    invoices = invoices.filter(
      (invoice) => invoice.studentId === parseInt(studentId)
    );
  }
  ...
  if (from_date) {
    ...
    invoices = invoices.filter(
      (invoice) => new Date(invoice.generated_at) >= fromDate
    );
  }
  ...
  if (to_date) {
    ...
    invoices = invoices.filter(
      (invoice) => new Date(invoice.generated_at) <= toDate
    );
  }
...
export const createInvoice = async (invoiceData) => {
  ...
  const invoiceNumber = `INV-${today.getFullYear()}-${String(newId).padStart(
    3,
    '0'
  )}`;
...
export const getMessages = async (queryParams = {}) => {
  ...
  if (filter === 'inbox') {
    messages = messages.filter(
      (msg) => msg.receiver_id === adminId && msg.receiver_type === 'admin'
    );
  } else if (filter === 'sent') {
    messages = messages.filter(
      (msg) => msg.sender_id === adminId && msg.sender_type === 'admin'
    );
  } else {
    messages = messages.filter(
      (msg) =>
...
export const getLeaveRequests = async (queryParams = {}) => {
  ...
  if (departmentId) {
    leaveRequests = leaveRequests.filter(
      (request) => request.departmentId === parseInt(departmentId)
    );
  }

  if (facultyId) {
    leaveRequests = leaveRequests.filter(
      (request) => request.facultyId === parseInt(facultyId)
    );
  }
  ...
  if (date_from) {
    ...
    leaveRequests = leaveRequests.filter(
      (request) => new Date(request.leave_date) >= fromDate
    );
  }
  ...
  if (date_to) {
    ...
    leaveRequests = leaveRequests.filter(
      (request) => new Date(request.leave_date) <= toDate
    );
  }
...
export const createLeaveRequest = async (leaveData) => {
  ...
  const department = mockDepartments.find(
    (d) => d.id === newRequest.departmentId
  );
...
export const updateLeaveStatus = async (leaveId, data) => {
  ...
  const department = mockDepartments.find(
    (d) => d.id === updatedLeave.departmentId
  );
