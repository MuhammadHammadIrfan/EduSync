...
import { getUpcomingEventsList } from "../../../utils/api/student"
...
export default function Events() {
  ...
  useEffect(() => {
    async function fetchEvents() {
      try {
        setLoading(true);
        const data = await getUpcomingEventsList();
        setEvents(data || []); // Ensure we always have at least an empty array
        setLoading(false);
      } catch (err) {
  ...
  const getFilteredEvents = () => {
    ...
    if (departmentFilter !== "all") {
      if (departmentFilter === "my-department") {
        // Filter for events specific to the student's department (CS in this case)
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <StudentSidebar activePage="upcoming-events" />
