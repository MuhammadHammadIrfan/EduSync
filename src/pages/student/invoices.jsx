...
import { FileText, Download, Calendar, User, Mail, Briefcase, Users, Building, Filter, Loader2, AlertCircle } from "lucide-react"
import StudentSidebar from "../../components/studentSiderbar"
import { getStudentInvoices } from "../../../utils/api/student"
...
useEffect(() => {
  async function fetchData() {
    try {
      setLoading(true);
      const invoicesData = await getStudentInvoices()
      console.log("Raw invoice data:", JSON.stringify(invoicesData, null, 2)); // Debug log
      setInvoices(invoicesData)
      
      // Set student information from the first invoice
      if (invoicesData && invoicesData.length > 0) {
        setStudent({
          name: invoicesData[0].student?.name || user?.name || 'Student',
          email: user?.email || '',
          studentId: user?.id || '',
          // Get department from user props or set a default department per major
          department: user?.departmentName || invoicesData[0].student?.department || 'Computer Science',
          // Access the class name directly from the student object
          class: invoicesData[0].student?.class || '',
          // Access the section name directly from the student object
          section: invoicesData[0].student?.section || ''
        });
      } else if (user) {
        // Set from user prop if no invoices
        setStudent({
