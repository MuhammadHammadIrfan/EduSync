...
export default function Users() {
  ...
  const handleCsvParse = (file) => {
    // Show some indication that file is being processed
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (!hasAllRequiredFields) {
          alert(
            `Invalid file format. The CSV must contain columns for: ${requiredFields.join(
              ', '
            )}`
          );
        ...
        if (invalidRows.length > 0) {
          alert(
            `Warning: ${invalidRows.length} rows have missing name or email values.`
          );
        }
  ...
  const handleImportSubmit = async (e) => {
    ...
    if (
      activeTab === 'students' &&
      (!importState.classId || !importState.sectionId)
    ) {
    ...
    try {
      ...
      // Prepare data for import API - use only relevant fields based on user type
      const importData = {
        type: activeTab === 'students' ? 'student' : 'faculty',
        departmentId: importState.departmentId,
        // Only include class/section for students
        ...(activeTab === 'students'
          ? {
      ...
      // Show results
      alert(
        `Import completed: ${result.success} users imported successfully, ${result.failed} failed.`
      );
            ...
            options: classes
              .filter(
                (cls) =>
                  ...
                  cls.departmentId === parseInt(filters.departmentId, 10)
              )
          ...
          },
          {
            ...
            options: sections
              .filter(
                (sec) =>
                  !filters.classId ||
                  sec.classId === parseInt(filters.classId, 10)
              )
      ...
      <div className='p-6 bg-gray-100 min-h-screen space-y-6'>
                    ...
                    >
                      ...
                      {sections
                        .filter(
                          (sec) =>
                            !formData.classId ||
                            sec.classId === parseInt(formData.classId, 10)
                        )
                        .map((sec) => (
        ...
        {showImportForm && (
          <div className='bg-white rounded-lg shadow-md p-6 mb-6'>
            <div className='flex justify-between items-center mb-4'>
              <h2 className='text-xl font-semibold text-gray-800'>
                Import {activeTab === 'students' ? 'Students' : 'Faculty'} from
                CSV
              </h2>
              <button
            ...
            <div className='mb-4 p-4 bg-blue-50 border-l-4 border-blue-500 flex items-start'>
              ...
              <div>
                ...
                <ul className='list-disc ml-5 text-sm text-blue-700'>
                  <li>
                    CSV file must contain <strong>name</strong> and{' '}
                    <strong>email</strong> columns only
                  </li>
                  <li>
                    Default password will be set to:{' '}
                    <strong>
                      {activeTab === 'students' ? 'student@123' : 'faculty@123'}
                    </strong>
                  </li>
                  <li>
                    Department
                    {activeTab === 'students' ? ', class, and section' : ''} are
                    selected below
                  </li>
                </ul>
              </div>
            ...
            <form onSubmit={handleImportSubmit} className='space-y-4'>
                ...
                <div>
                  ...
                  >
                    ...
                    {classes
                      .filter(
                        (cls) =>
                          !importState.departmentId ||
                          cls.departmentId ===
                            parseInt(importState.departmentId, 10)
                      )
                      .map((cls) => (
                        <option key={cls.id} value={cls.id}>
                          {cls.name}
                  ...
                  >
                    ...
                    {sections
                      .filter(
                        (sec) =>
                          !importState.classId ||
                          sec.classId === parseInt(importState.classId, 10)
                      )
                      .map((sec) => (
                        <option key={sec.id} value={sec.id}>
                          {sec.name}
              ...
              <div className='mt-4'>
                ...
                <div
                  className={`border-2 border-dashed p-4 rounded-md transition-colors ${
                    importState.csvData
                ...
                >
                  <div className='flex flex-col items-center justify-center'>
                    {!importState.csvData ? (
                      <>
                        ...
                        <button
                          ...
                          className='px-4 py-2 bg-blue-100 text-blue-700 rounded hover:bg-blue-200 mb-2'
                        >
                          Select CSV File
                        </button>
                      </>
                    ) : (
                      <p className='mb-2 text-green-600 font-medium flex items-center'>
                        <svg
                    ...
                    <input
                      ...
                      className={importState.csvData ? 'hidden' : 'w-full'}
                    />
              ...
              {/* Buttons */}
              // Only showing the modified section - Import form button area
              <div className='flex justify-end space-x-2 pt-4'>
                <button
                  type='button'
                  onClick={() => setShowImportForm(false)}
